...
const Settings = ({ usersData, setUsersData }) => {
  ...
  const saveNewUser = () => {
    if (!addForm.name || !addForm.email || !addForm.password) return alert('Name, Email and Password required');
        ...
        <div className="bg-white dark:bg-zinc-900/80 backdrop-blur-md border border-gray-200 dark:border-zinc-800 rounded-xl overflow-hidden shadow-sm dark:shadow-none transition-colors duration-200">
          <div className="p-5 border-b border-gray-200 dark:border-zinc-800 bg-gray-50 dark:bg-transparent flex justify-between items-center transition-colors">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-zinc-100 flex items-center">
              <Shield className="mr-2 text-asaka-500" size={18} />
              Access Control
            </h3>
            <button 
                ...
                {isAdding && (
                  <tr className="bg-gray-50 dark:bg-zinc-800/30">
                    <td className="px-6 py-3">
                      <input type="text" placeholder="Full Name" onChange={e => setAddForm({...addForm, name: e.target.value})} className="bg-white dark:bg-zinc-900 border border-gray-300 dark:border-zinc-700 text-gray-900 dark:text-zinc-100 rounded w-full p-1.5 mb-2 outline-none focus:border-asaka-500 focus:ring-1 focus:ring-asaka-500 block" />
                      <input type="email" placeholder="Email Address" onChange={e => setAddForm({...addForm, email: e.target.value})} className="bg-white dark:bg-zinc-900 border border-gray-300 dark:border-zinc-700 text-gray-900 dark:text-zinc-100 rounded w-full p-1.5 mb-2 outline-none focus:border-asaka-500 focus:ring-1 focus:ring-asaka-500 block" />
                      <input type="password" placeholder="Password" onChange={e => setAddForm({...addForm, password: e.target.value})} className="bg-white dark:bg-zinc-900 border border-gray-300 dark:border-zinc-700 text-gray-900 dark:text-zinc-100 rounded w-full p-1.5 outline-none focus:border-asaka-500 focus:ring-1 focus:ring-asaka-500 block" />
                    </td>
                    <td className="px-6 py-3">
                      <select onChange={e => setAddForm({...addForm, role: e.target.value})} defaultValue="Staff" className="bg-white dark:bg-zinc-900 border border-gray-300 dark:border-zinc-700 text-gray-900 dark:text-zinc-100 rounded p-1.5 outline-none focus:border-asaka-500 focus:ring-1 focus:ring-asaka-500">
                    ...
                    <td className="px-6 py-3">
                      <select onChange={e => setAddForm({...addForm, status: e.target.value})} defaultValue="Active" className="bg-white dark:bg-zinc-900 border border-gray-300 dark:border-zinc-700 text-gray-900 dark:text-zinc-100 rounded p-1.5 outline-none focus:border-asaka-500 focus:ring-1 focus:ring-asaka-500">
                ...
                {usersData.map((user) => (
                  <tr key={user.id} className="hover:bg-gray-50 dark:hover:bg-zinc-800/20 transition-colors">
                    {editingId === user.id ? (
                      <>
                        <td className="px-6 py-3">
                          <input type="text" value={editForm.name} onChange={e => setEditForm({...editForm, name: e.target.value})} className="bg-white dark:bg-zinc-900 border border-gray-300 dark:border-zinc-700 text-gray-900 dark:text-zinc-100 rounded w-full p-1.5 mb-2 outline-none focus:border-asaka-500 focus:ring-1 focus:ring-asaka-500 block" />
                          <input type="email" value={editForm.email} onChange={e => setEditForm({...editForm, email: e.target.value})} className="bg-white dark:bg-zinc-900 border border-gray-300 dark:border-zinc-700 text-gray-900 dark:text-zinc-100 rounded w-full p-1.5 mb-2 outline-none focus:border-asaka-500 focus:ring-1 focus:ring-asaka-500 block" />
                          <input type="password" placeholder="New Password (optional)" onChange={e => setEditForm({...editForm, password: e.target.value})} className="bg-white dark:bg-zinc-900 border border-gray-300 dark:border-zinc-700 text-gray-900 dark:text-zinc-100 rounded w-full p-1.5 outline-none focus:border-asaka-500 focus:ring-1 focus:ring-asaka-500 block" />
                        </td>
                        <td className="px-6 py-3">
                          <select value={editForm.role} onChange={e => setEditForm({...editForm, role: e.target.value})} className="bg-white dark:bg-zinc-900 border border-gray-300 dark:border-zinc-700 text-gray-900 dark:text-zinc-100 rounded p-1.5 outline-none focus:border-asaka-500 focus:ring-1 focus:ring-asaka-500">
                            <option value="Admin">Admin</option>
                            <option value="Manager">Manager</option>
                            <option value="Staff">Staff</option>
                          </select>
                      ...
                      <>
                        ...
                        <td className="px-6 py-3 text-right align-middle">
                          {user.role !== 'Admin' || usersData.filter(u => u.role === 'Admin').length > 1 ? (
                            <>
                              <button onClick={() => startEdit(user)} className="text-gray-400 dark:text-zinc-500 hover:text-asaka-500 dark:hover:text-asaka-300 mr-3 transition-colors"><Edit2 size={16} /></button>
          ...
          <div className="bg-white dark:bg-zinc-900/80 backdrop-blur-md border border-gray-200 dark:border-zinc-800 rounded-xl p-5 shadow-sm dark:shadow-none transition-colors duration-200">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" defaultValue="Asaka Sushi - Sidi Maarouf" className="w-full bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 text-gray-900 dark:text-zinc-100 rounded px-3 py-2 text-sm outline-none focus:border-asaka-500 focus:ring-1 focus:ring-asaka-500 transition-colors" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-500 dark:text-zinc-400 mb-1">Contact Phone</label>
                <input type="text" defaultValue="+212 522 00 00 00" className="w-full bg-gray-50 dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 text-gray-900 dark:text-zinc-100 rounded px-3 py-2 text-sm outline-none focus:border-asaka-500 focus:ring-1 focus:ring-asaka-500 transition-colors" />
              </div>
