...
import {
  ...
  Phone, MessageSquare, Ban, Check, RefreshCw, X, ShoppingBag,
} from 'lucide-react';
...
const PLATFORM_COLORS = {
  ...
  Direct:   'bg-asaka-500/15 text-asaka-300 border-asaka-500/30',
          ...
          {(order.rawItems || order.payload) && (
            <div className="bg-zinc-950/40 rounded-xl p-3 border border-zinc-800/40">
              <h4 className="text-xs font-bold text-zinc-400 uppercase tracking-widest mb-2 flex items-center gap-1.5">
                <ShoppingBag size={12} className="text-zinc-500" /> Détails de la commande
              </h4>
              
              {/* Items */}
              {order.rawItems && order.rawItems.length > 0 && (
                <div className="space-y-1.5 mb-3 pb-3 border-b border-zinc-800/40">
                  {order.rawItems.map((c, i) => (
                    <div key={i} className="flex justify-between text-sm">
                      <span className="text-zinc-300">
                        <span className="font-bold text-emerald-400 mr-2">{c.qty}x</span>
                        {c.item.name}
                      </span>
                      <span className="text-zinc-500">{c.item.price * c.qty} DH</span>
                    </div>
                  ))}
                </div>
              )}
              ...
              {order.payload && (
                <div className="space-y-1.5 text-xs">
                  <div className="flex justify-between text-zinc-400">
                    ...
                    <span>{order.payload.subtotal} DH</span>
                  </div>
                  {order.payload.deliveryFee > 0 && (
                    <div className="flex justify-between text-zinc-400">
                      <span>Frais de livraison</span>
                      <span>+{order.payload.deliveryFee} DH</span>
                    </div>
                  )}
                  {order.payload.tip > 0 && (
                    <div className="flex justify-between text-emerald-400/80">
                      <span>Pourboire livreur</span>
                      <span>+{order.payload.tip} DH</span>
                    </div>
                  )}
                  {order.payload.offerDiscount > 0 && (
                    <div className="flex justify-between text-rose-400">
                      <span>Remise (Offre)</span>
                      <span>-{order.payload.offerDiscount} DH</span>
                    </div>
                  )}
                  {order.payload.pointsDiscount > 0 && (
                    <div className="flex justify-between text-rose-400">
                      <span>Remise (Points fidélité)</span>
                      <span>-{order.payload.pointsDiscount} DH</span>
                    </div>
                  )}
                  <div className="flex justify-between font-bold text-zinc-200 pt-2 mt-2 border-t border-zinc-800/40 text-sm">
                    <span>Total à payer</span>
                    <span className="text-emerald-400">{order.payload.roundedTotal} DH</span>
                  </div>
                </div>
              )}
            </div>
